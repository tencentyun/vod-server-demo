...
export class PlayController {
    ...
    public async playVideo(@Body({validate:false}) dto: PlayVideoDTO) {
        ...
        try {
            ...
            logger.error(`[${requestId}] play video success: `, video);

            // 获取播放信息
            let playInfo = GetPlayInfo(video!.Url, 2*3600);
            ...
            logger.info(`[${ctx.RequestId}] playVideo error:`, error);
            logger.info(`[${ctx.RequestId}] playVideo dto:`, dto);
            logger.error(`[${requestId}] play video fail: ${error.Code}, ${error.Message}, error:`, error);
